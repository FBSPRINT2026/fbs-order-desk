  ...
  return (
    <div className={`labels-page ${size === "letter" ? "sz-letter" : "sz-4x6"}`}>
      <style>{`
        ...
        .lb-ship { border: 1px solid #111; padding: 3px 5px; white-space: pre-line; font-size: 1em; }
      ...
      {Array.from({ length: boxes }, (_, bi) => (
        <div className={"label" + (size === "4x6" || bi % 2 === 1 ? " brk" : "")} key={bi}>
          ...
          <div>
            <div className="lb-cust">{cust.company || cust.name || "No customer"}</div>
            {cust.company && cust.name && <div>{cust.name}{cust.phone ? ` · ${cust.phone}` : ""}</div>}
